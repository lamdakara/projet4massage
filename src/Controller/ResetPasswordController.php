...
class ResetPasswordController extends AbstractController
{
    ...
    /**
     * @Route("/motdepasseoublie", name="reset_password")
     */
    public function index(Request $request, MailjetService $mailjetService)
    {
        if ($this->getUser()) {
        ...
        if ($request->get('email')) {
            ...
            if ($user) {
                ...
                $content = "Bonjour " . $user->getFirstname() . "<br/>Vous avez demandé à réinitialiser votre mot de passe sur le site Lili Giroud - massages madérothérapeutique.<br/><br/>";
                $content .= "Merci de bien vouloir cliquer sur le lien suivant pour <a href='" . $url . "'>mettre à jour votre mot de passe</a>.";

                $mailjetService->send($user->getEmail(), $user->getFirstname() . ' ' . $user->getLastname(), 'Réinitialiser votre mot de passe sur le site Lili Giroud - massages madérothérapeutique', $content);
    ...
    /**
     * @Route("/modifiermotdepasseoublie/{token}", name="update_password")
     */
    public function update(Request $request, $token, UserPasswordHasherInterface $encoder)
    {
        // Récuperation dans le BDD
